...
private:
	std::atomic<std::uint32_t> refCount_; // 引用计数
	// waiter_的含义
...
public:
	...
	~shared_task_promise() {
		...
		{
			std::launder(reinterpret_cast<T*>(&valueStorage))->~T();// 显示调用析构函数
		}
	}

	// return_value 用于构造并返回一个值
	template<typename Value>
	requires std::is_convertible_v<Value&&,T>  // 检查是否可以通过Value&&隐式转换成T
	void return_value(Value&& value) noexcept(std::is_nothrow_constructible_v<T, Value&&>) {
		new (&valueStorage) T(std::forward<Value>(value));  // 在 valueStorage 上构造 T
	}
...
public:
	...
	bool is_ready()const noexcept { return !coroutine_ || coroutine_.promise().is_ready(); }
...
template<typename U>
inline bool operator==(const shared_task<U>&l, const shared_task<U>&r) noexcept
{
	return l.coroutine_ == r.coroutine_;
}
template<typename T>
void swap(shared_task<T>& a,shared_task<T>&b) {
	a.swap(b);
}