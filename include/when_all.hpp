  ...
  when_all_latch(when_all_latch &&other)
      : m_count(other.m_count.load(std::memory_order::acquire)),
        m_awaiting_coroutine(
            std::exchange(other.m_awaiting_coroutine, nullptr)) {}
...
template <typename task_container_type> class when_all_ready_awaitable;

template <typename return_type> class when_all_task;
...
public:
  explicit when_all_ready_awaitable(task_types &&...tasks) noexcept(
      std::conjunction<
          std::is_nothrow_move_constructible<task_types>...>::value)
...
private:
  ...
  auto try_await(std::coroutine_handle<> awaiting_coroutine) noexcept -> bool {
    std::apply([this](auto &&...tasks) { ((tasks.start(m_latch)), ...); },
               m_tasks);
...
template <typename return_type> class when_all_task_promise {
public:
...
public:
  ...
  template <typename task_container_type> friend class when_all_ready_awaitable;
...
template <concepts::awaitable awaitable,
          typename return_type =
              concepts::awaitable_traits<awaitable &&>::await_result_t>
...
[[nodiscard]] auto when_all(awaitables_type... awaitables) {
  return when_all_ready_awaitable<
      std::tuple<when_all_task<typename concepts::awaitable_traits<
          awaitables_type>::await_result_t>...>>(
...
template <
    ...
    typename return_type =
        typename concepts::awaitable_traits<awaitable_type>::await_result_t>
